...
import zod from "zod";
import { tryParsePublicKey } from "./ton.wallet";
...
export class TonProofService {
  ...
  public async checkProof(payload: CheckProofRequestDto, getWalletPublicKey: (address: string) => Promise<Buffer | null>): Promise<boolean> {
    try {
      ...
      //           Timestamp ++
      //           Payload
      const msg = Buffer.concat([Buffer.from(tonProofPrefix), wc, message.address, dl, Buffer.from(message.domain.value), ts, Buffer.from(message.payload)]);
